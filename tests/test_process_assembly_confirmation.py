...
from scripts import smoke_process_assembly_confirmation as smoke
...
def test_freeze_matches_inputs_and_resumes_without_historical_mounts(
    ...
    assert "dependency_confirmation" not in plan
    ...
    assert (
        report["historical_accounting"]["assembly"]["outer_sign_normalized_slots"]
        is None
    )
...
def test_real_repair_sign_and_two_gain_handoffs_are_fresh_and_counted_once(
    ...
    assert accounting["assembly"]["outer_sign_normalized_slots"] == 1
    assert accounting["assembly"]["conversion_rejected_attempts"] == 1
    assert accounting["assembly"]["intrinsic_factor_confirmations"] == 0
    ...
    law = route["committed_slot_decisions"][0]
    assert law["raw_reply"]["expression"] == "-k*h_up"
    assert "-" not in law["normalized_reply"]["expression"]
    # Missing saved usage is visible, not counted as a measured zero-cost repair.
    repair = next(source for source in calls if "selected_term" in source["payload"])
    assert repair
    base = root / "construction/results" / tasks[0]["construction_task"]["task_id"]
    f = sealed_read(base / "review/function_stage.json")["result"]
    key = next(
        e["request_hash"] for e in f["events"] if e["step"].startswith("atomic_repair_")
    )
...
def test_empty_optional_process_remains_valid(historical, tmp_path):
    ...
    client = experiment.pilot.make_client(
        ...
        task,
        "offline",
        transport=smoke.independent_transport([]),
    )
