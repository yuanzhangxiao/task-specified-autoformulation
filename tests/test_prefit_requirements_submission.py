...
""",
    ...
    def run(**overrides):
        return subprocess.run(
            ["bash", str(repo / "scripts/hpc/submit_prefit_requirements_aces.sh")],
            env={**env, **overrides},
            capture_output=True,
            text=True,
        )
...
def test_shared_server_selects_fit_free_cli():
    result = subprocess.run(
        [
            ...
            "--check-config",
            "configs/prefit_requirement_feedback_v1.json",
        ],
        capture_output=True,
        text=True,
    )
    assert result.returncode == 0, result.stderr
    assert result.stdout.strip() == "prefit_requirement_campaign.py"
...
def test_worker_surfaces_preflight_failure_in_job_stderr(tmp_path):
    ...
    result = subprocess.run(
        ["bash", str(repo / "scripts/hpc/run_prefit_requirements_aces.sh"), "prepare"],
        env={
            **os.environ,
            "PATH": str(bins) + os.pathsep + os.environ["PATH"],
            "AF_REPO_ROOT": str(repo),
            ...
            "AF_CONFIG": str(repo / "configs/prefit_requirement_feedback_v1.json"),
