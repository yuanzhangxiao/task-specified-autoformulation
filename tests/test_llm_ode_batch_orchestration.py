"""Run the LLM-ODE batch script locally against stubs.
...
BATCH = Path("scripts/hpc/phase_b_llm_ode_vllm_batch.slurm").resolve()

WORKER = '''#!/usr/bin/env python3
"""Stand-in for scripts/phase_b_llm_ode.py that records what it was asked."""
import json, os, sys
from pathlib import Path
...
if command == "prepare":
    ...
    assert os.environ.get("AF_LLM_ODE_ROOT"), \\
...
def _checkout(path: Path) -> str:
    ...
    (path / "llmode").mkdir(parents=True)
    (path / "llmode" / "llmode.py").write_text("", encoding="utf-8")
...
def cluster(tmp_path: Path) -> dict:
    ...
    worker = repo / "scripts" / "phase_b_llm_ode.py"
    ...
    environment = {
        **os.environ,
        ...
        "AF_LLM_ODE_OUTPUT_ROOT": str(output),
        "AF_LLM_ODE_ROOT": str(upstream),
        "AF_LLM_ODE_CONFIG": str(config),
...
def test_a_checkout_at_the_wrong_commit_fails_before_the_model_loads(
    ...
    result = _run(cluster, AF_LLM_ODE_CONFIG=str(config))
...
def test_a_missing_checkout_fails_before_any_gpu_work(cluster: dict) -> None:
    ...
    del environment["AF_LLM_ODE_ROOT"]
