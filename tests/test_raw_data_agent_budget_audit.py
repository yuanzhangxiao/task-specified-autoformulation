...
def test_audit_detects_provider_tool_limit_excess(tmp_path: Path) -> None:
    ...
    assert row["raw_processed_code_interpreter_calls"] == 3
    assert row["raw_nonterminal_code_interpreter_records"] == 0
    assert row["limit_exceeded"] is True


def test_audit_does_not_count_interpreting_record_as_processed(
    tmp_path: Path,
) -> None:
    compact, candidate = _candidate()
    run = tmp_path / "run"
    ...
    assert row["raw_processed_code_interpreter_calls"] == 2
    assert row["raw_nonterminal_code_interpreter_records"] == 1
    assert row["limit_exceeded"] is False
    assert row["artifact_count_disagrees_with_processed_count"] is True
