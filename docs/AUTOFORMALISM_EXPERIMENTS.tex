...
The T1 task is to predict blood-glucose responses to meal timing and amount,
using permitted auxiliary measurements. We examine its anonymized canonical
version. Our intervention changes initial tissue glucose by $\pm20\%$, while
holding initial blood glucose and the meal schedule fixed. Blood and tissues
exchange glucose: two simulated subjects with the same initial blood reading
can therefore evolve differently. This is a synthetic starting-condition test,
not a specific therapy. The simulator regenerates the supplied auxiliary
trajectories; the learned models keep their fitted parameters fixed.

Assisted Autoformalism R9, a no-specification model, and validation-selected
Sol~0 have similar training NMSEs (0.0416, 0.0436, and 0.0368).
Their equations nevertheless predict different responses. No-specification
ignores tissue glucose and must predict no change; R9 retains a positive
tissue-to-blood contribution, while Sol~0 exaggerates the response.
For fasting with 20\% less initial tissue glucose, normalized errors in the
change from each model's own unperturbed control are 0.126, 1.000, and 0.901,
respectively; predicting no change scores 1.

Thus, comparable training accuracy can conceal different intervention responses.
R9 received assisted sign repair, refitting, and pruning, and does not uniformly
beat Sol on absolute trajectory error. Tissue-glucose dependence is optional
under T1: this example motivates testing learned dependencies, but does not
establish a benefit from the task specification. Appendix~\ref{app:interventional_discrimination}
provides equations, trajectory plots, and results for all Sol repetitions.
