...
def main() -> None:
    ...
    for benchmark_id in benchmark_ids:
        ...
        if tier not in {"easy", "hard"}:
            raise ValueError(f"cannot infer tier from benchmark ID: {benchmark_id}")
        config = DataConfig(
        ...
        condition = float(
            train_singular[0] / max(float(train_singular[claimed_index]), 1e-15)
        )
        rows.append(
            {
                "benchmark_id": benchmark_id,
        ...
        print(
            ...
            f"train_rank={train_rank} test_rank={test_rank} condition={condition:.3g}",
            flush=True,
        )
...
def _semantic_pair_checks(
    ...
    for row in rows:
        ...
        if benchmark_id not in matrices:
            continue
        key = (
            str(row["family"]),
            str(row["task"]),
