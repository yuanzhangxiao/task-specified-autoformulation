...
The nine cases are equally weighted, so a method is not rewarded for the
cases it happens to cover. Where a method has no evaluable run for one of
them, its headline aggregate is unavailable rather than computed over a
different set of cases, which the notes forbid explicitly.
"""
...
def case_median(values: list[float]) -> float | None:
    """Median over a case's repetitions; None when none were evaluable."""
...
def macro(case_values: list[float | None]) -> tuple[float | None, float | None]:
    ...
    A method missing a whole case has no headline aggregate. Computing one
    over the cases it did cover would compare methods on different rosters.
    """
    ...
    return centre, median([abs(value - centre) for value in present])
    ...
    for method in sorted({str(row["method"]) for row in rows}):
        ...
        for case in NINE_CASES:
            samples = [
                ...
                and str(row["benchmark_id"]) == case
                and isinstance(row.get(field), (int, float))
            ]
            values.append(case_median(samples))
...
def main() -> None:
    ...
    for method, key in sorted(METHOD_KEYS.items()):
        ...
        if absent:
            notes.append(
                f"{method}: no evaluable run for {len(absent)} of nine cases "
        ...
        if centre is not None:
            notes.append(
                f"{method}: mechanism compliance {render(centre, spread)} "
