...
def submit(root: Path, index: int = 0) -> dict:
    ...
    with public._lock(root / "scheduler"):
        ...
        if "assembly_confirmation" in plan:
            value["experiment"] = plan["assembly_confirmation"]["protocol"]
            value["fresh_constructions"] = plan["assembly_confirmation"][
                "fresh_constructions"
            ]
        if "revision_confirmation" in plan:
            value["experiment"] = plan["revision_confirmation"]["protocol"]
            value["fresh_constructions"] = plan["revision_confirmation"][
                "fresh_constructions"
            ]
