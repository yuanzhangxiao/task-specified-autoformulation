#!/usr/bin/env python3
"""Supervise fitter-mesh experiments; summarize with the stdlib."""
...
def summarize(output: Path) -> None:
    ...
    lines = [
        ...
        "",
        "Previous/handoff/mapped compare the same full mesh. Mesh12000/24000 "
        "retain all observations and input corners. More_time is a separate larger "
        "initializer-budget reference, not an equal-budget speed comparison.",
        "Matched controls share public time grids/counts, "
        "not public forcing complexity.",
        ...
        "Validation NMSE | Clean validation NMSE | Recovered | Variables | Setup s | "
        "First iteration s | Rejected trials |",
        "|---|---|---|---|---:|---:|---:|---:|---:|---|---:|---:|---:|---:|",
    ]
    for task in frozen["tasks"]:
        ...
        lines.append(
            ...
            f"{progress.get('decision_variables')} | "
            f"{progress.get('construction_seconds')} | "
            f"{(progress.get('iterations') or [{}])[0].get('seconds')} | "
            f"{refinement.get('rejected_sensitivity_trials')} |"
        )
        ...
        diagnostics.append(
            {
                ...
                "mesh": initial.get("mesh"),
                "collocation_variables": progress.get("decision_variables"),
                "collocation_first_iteration": (progress.get("iterations") or [{}])[0],
                "rejected_sensitivity_trials": refinement.get(
                    "rejected_sensitivity_trials"
                ),
                "stages": refinement.get("stages", []),
                "poll_handoff": (
                    json.loads((directory / "recovery/poll_handoff.json").read_text())
                    if (directory / "recovery/poll_handoff.json").exists()
                    else None
                ),
...
def main() -> None:
    ...
    if args.stage == "prepare":
        plan = (
            MeshExperiment.model_validate(json.loads(args.config.read_text()))
            if args.config
            else MeshExperiment()
        )
        ...
        for kind in ("shared", "piecewise"):
            ...
            result = fit_collocation_forward_sensitivity(
                ...
                CollocationSensitivityConfig(
                    ...
                    collocation_target_variables=120,
            ...
            if (
                ...
                raise SystemExit(f"Failed mesh smoke: {kind}")
