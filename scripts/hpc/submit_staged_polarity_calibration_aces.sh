#!/bin/bash
# Freeze and submit the one-H100 topology-stage polarity calibration on ACES.
...
: "${AF_OUTPUT_ROOT:=${SCRATCH}/phase_b/staged-polarity-calibration-v1-aces-h100x1}"
: "${AF_POLARITY_CONFIG:=${AF_REPO_ROOT}/configs/staged_polarity_calibration_v1.json}"
: "${AF_VLLM_IMAGE:=${PROJECT:-${SCRATCH}}/containers/vllm-openai-v0.27.1.sif}"
: "${AF_HF_HOME:=${SCRATCH}/huggingface-cache}"
: "${AF_COMPUTE_CACHE_ROOT:=${SCRATCH}/autoformalism-runtime-cache/staged-polarity}"
: "${AF_IPC_TMP_ROOT:=${SCRATCH}/af-ipc}"
: "${AF_WORKER_JOB:=${AF_REPO_ROOT}/scripts/hpc/staged_polarity_calibration_aces.slurm}"
...
for path in "${AF_PYTHON}" "${AF_POLARITY_CONFIG}" "${AF_VLLM_IMAGE}" "${AF_WORKER_JOB}"; do
  [[ -e "${path}" ]] || { echo "missing polarity-calibration input: ${path}" >&2; exit 2; }
done
...
[[ -z "$(git -C "${AF_REPO_ROOT}" status --porcelain)" ]] || {
  echo "polarity-calibration checkout must be clean before freezing" >&2
  exit 2
}
mkdir -p "${AF_OUTPUT_ROOT}/logs" "${AF_HF_HOME}" "${AF_COMPUTE_CACHE_ROOT}" "${AF_IPC_TMP_ROOT}"
cd "${AF_REPO_ROOT}"
"${AF_PYTHON}" scripts/staged_polarity_calibration.py freeze --config "${AF_POLARITY_CONFIG}" --output "${AF_OUTPUT_ROOT}/plan.json"
mkdir "${submission_intent}"
jq -n --arg created_at_utc "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg commit "$(git rev-parse HEAD)" --arg plan_sha256 "$(sha256sum "${AF_OUTPUT_ROOT}/plan.json" | awk '{print $1}')" '{schema_version:"staged-polarity-submission-intent-1",status:"sbatch_pending_or_uncertain",created_at_utc:$created_at_utc,commit:$commit,plan_sha256:$plan_sha256}' >"${submission_intent}/intent.json"
...
submission="$(sbatch --parsable --account="${AF_ACES_ACCOUNT}" --output="${AF_OUTPUT_ROOT}/logs/staged-polarity-%j.out" --error="${AF_OUTPUT_ROOT}/logs/staged-polarity-%j.err" --export="${common_export}" "${AF_WORKER_JOB}")"
readonly job_id="${submission%%;*}"
printf '%s\n' "${job_id}" >"${submission_intent}/job_id.txt"
temporary="${submission_manifest}.tmp"
jq -n --arg submitted_at_utc "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg job_id "${job_id}" --arg plan_sha256 "$(sha256sum "${AF_OUTPUT_ROOT}/plan.json" | awk '{print $1}')" --arg commit "$(git rev-parse HEAD)" '{schema_version:"staged-polarity-submission-1",submitted_at_utc:$submitted_at_utc,platform:"aces-h100x1",job_id:$job_id,plan_sha256:$plan_sha256,commit:$commit,parameter_fitting_performed:false,scientific_judge_called:false,test_data_opened:false,private_reference_opened:false}' >"${temporary}"
mv "${temporary}" "${submission_manifest}"
echo "ACES_STAGED_POLARITY_JOB=${job_id}"
echo "ACES_STAGED_POLARITY_ROOT=${AF_OUTPUT_ROOT}"
