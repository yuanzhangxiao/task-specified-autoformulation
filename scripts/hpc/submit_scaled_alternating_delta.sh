#!/bin/bash
# One final paired matrix. Never create additional attempts after its results.
set -euo pipefail
...
export AF_SOURCE_ROOT="${AF_SOURCE_ROOT:-/work/hdd/bibo/yxiao2/phase_b/fitter-final-alternatives-v1}"
export AF_OUTPUT_ROOT="${AF_OUTPUT_ROOT:-/work/hdd/bibo/yxiao2/phase_b/fitter-scaled-alternating-v1}"
...
"${AF_PYTHON}" scripts/run_scaled_alternating.py prepare --source "${AF_SOURCE_ROOT}" --output "${AF_OUTPUT_ROOT}" --config "${AF_CONFIG:-configs/fitter_scaled_alternating_v1.json}"
...
r.update({sys.argv[2]:sys.argv[3], 'commit':sys.argv[4], 'cpus_per_task':1,
          'fit_memory_gb':64, 'fit_tasks':2, 'gpus':0, 'no_automatic_followup':True,
...
gate="$(submit gate --mem=16G --time=00:15:00 scripts/hpc/scaled_alternating_delta.slurm gate)"
fit="$(submit fit --dependency="afterok:${gate}" --kill-on-invalid-dep=yes --array="0-1%${concurrency}" scripts/hpc/scaled_alternating_delta.slurm run)"
submit summary --mem=2G --time=00:10:00 --dependency="afterany:${fit}:${gate}" --kill-on-invalid-dep=yes scripts/hpc/scaled_alternating_delta.slurm summarize >/dev/null
cat "${AF_OUTPUT_ROOT}/submission.json"
