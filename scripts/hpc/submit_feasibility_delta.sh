#!/bin/bash
# Paired feasibility recovery and model-feature diagnostics: one arm per CPU task.
set -euo pipefail
...
export AF_SOURCE_ROOT="${AF_SOURCE_ROOT:-/work/hdd/bibo/yxiao2/phase_b/physical-initialization-v1}"
export AF_OUTPUT_ROOT="${AF_OUTPUT_ROOT:-/work/hdd/bibo/yxiao2/phase_b/fitter-feasibility-v1}"
...
"${AF_PYTHON}" scripts/run_feasibility_campaign.py prepare --source "${AF_SOURCE_ROOT}" --output "${AF_OUTPUT_ROOT}" --config configs/fitter_feasibility_v1.json
...
smoke="$(sbatch --parsable --time=00:10:00 --output="${AF_OUTPUT_ROOT}/logs/smoke-%j.out" --error="${AF_OUTPUT_ROOT}/logs/smoke-%j.err" --export=ALL scripts/hpc/feasibility_delta.slurm smoke)"
smoke="${smoke%%;*}"
record_job smoke_job_id "${smoke}"
fit="$(sbatch --parsable --dependency="afterok:${smoke}" --kill-on-invalid-dep=yes --array="0-${last}%${concurrency}" --output="${AF_OUTPUT_ROOT}/logs/fit-%A_%a.out" --error="${AF_OUTPUT_ROOT}/logs/fit-%A_%a.err" --export=ALL scripts/hpc/feasibility_delta.slurm run)"
fit="${fit%%;*}"
record_job fit_job_id "${fit}"
summary="$(sbatch --parsable --dependency="afterany:${fit}" --time=00:10:00 --output="${AF_OUTPUT_ROOT}/logs/summary-%j.out" --error="${AF_OUTPUT_ROOT}/logs/summary-%j.err" --export=ALL scripts/hpc/feasibility_delta.slurm summarize)"
summary="${summary%%;*}"
