...
case "${1:?stage}" in
 prepare)
    ...
    "$AF_PYTHON" -m pytest -q -p no:cacheprovider tests/test_fresh_shared.py tests/test_fresh_shared_submission.py tests/test_review_multi.py tests/test_review_integrity.py tests/test_process_pruning.py
    "$AF_PYTHON" scripts/smoke_fresh_shared.py
...
PY
    ;;
 dispatch) exec "$AF_PYTHON" scripts/submit_fresh_shared.py --root "$AF_OUTPUT_ROOT" --round "$AF_ROUND" ;;
 prune) exec "$AF_PYTHON" scripts/fresh_shared.py prune-task --root "$AF_OUTPUT_ROOT" ;;
 report) exec "$AF_PYTHON" scripts/fresh_shared.py report --root "$AF_OUTPUT_ROOT" ;;
