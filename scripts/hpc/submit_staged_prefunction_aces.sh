#!/bin/bash
# Freeze and submit the six-task public pre-function integration on ACES.
...
: "${AF_OUTPUT_ROOT:=${SCRATCH}/phase_b/staged-prefunction-integration-v1-aces-h100x1}"
: "${AF_PREFUNCTION_CONFIG:=${AF_REPO_ROOT}/configs/staged_prefunction_integration_v1.json}"
...
: "${AF_COMPUTE_CACHE_ROOT:=${SCRATCH}/autoformalism-runtime-cache/staged-prefunction}"
: "${AF_IPC_TMP_ROOT:=${SCRATCH}/af-ipc}"
: "${AF_WORKER_JOB:=${AF_REPO_ROOT}/scripts/hpc/staged_prefunction_aces.slurm}"
...
for path in "${AF_PYTHON}" "${AF_PREFUNCTION_CONFIG}" "${AF_PUBLIC_ROOT}" "${AF_VLLM_IMAGE}" "${AF_WORKER_JOB}"; do
  [[ -e "${path}" ]] || { echo "missing prefunction input: ${path}" >&2; exit 2; }
done
...
[[ -z "$(git -C "${AF_REPO_ROOT}" status --porcelain)" ]] || {
  echo "prefunction checkout must be clean before freezing" >&2
  exit 2
}
...
submission="$(sbatch --parsable --account="${AF_ACES_ACCOUNT}" --output="${AF_OUTPUT_ROOT}/logs/staged-prefunction-%j.out" --error="${AF_OUTPUT_ROOT}/logs/staged-prefunction-%j.err" --export="${common_export}" "${AF_WORKER_JOB}")"
readonly job_id="${submission%%;*}"
printf '%s\n' "${job_id}" >"${submission_intent}/job_id.txt"
temporary="${submission_manifest}.tmp"
jq -n --arg submitted_at_utc "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg job_id "${job_id}" --arg plan_sha256 "$(sha256sum "${AF_OUTPUT_ROOT}/plan.json" | awk '{print $1}')" --arg commit "$(git rev-parse HEAD)" '{schema_version:"staged-prefunction-submission-1",submitted_at_utc:$submitted_at_utc,platform:"aces-h100x1",job_id:$job_id,plan_sha256:$plan_sha256,commit:$commit,function_generation_performed:false,parameter_fitting_performed:false,scientific_judge_called:false,test_data_opened:false,private_reference_opened:false}' >"${temporary}"
mv "${temporary}" "${submission_manifest}"
echo "ACES_STAGED_PREFUNCTION_JOB=${job_id}"
echo "ACES_STAGED_PREFUNCTION_ROOT=${AF_OUTPUT_ROOT}"
