#!/bin/bash
# Import a retained global round and queue a fresh bounded development phase.
set -euo pipefail
...
: "${AF_CONTINUATION_PROTOCOL:=review-deadline-3}"
...
"$AF_PYTHON" "$AF_REPO_ROOT/scripts/review_deadline.py" prepare-continuation \
  ...
  --continuation-protocol "$AF_CONTINUATION_PROTOCOL"
