...
set -euo pipefail
round="${1:-0}"
[[ "$#" -le 1 && "$round" =~ ^([0-9]|1[0-4])$ ]] || { echo 'Usage: submit_fresh_shared_aces.sh [0..14]' >&2; exit 2; }
...
export AF_OUTPUT_ROOT="${AF_OUTPUT_ROOT:-$group/dalla-shared-multi-pruning-v1}"
export AF_PUBLIC_ROOT="${AF_PUBLIC_ROOT:-$group/dalla-response-feedback-r14-v1/public}"
export AF_CONFIG="$AF_REPO_ROOT/configs/dalla_shared_multi_pruning_v1.json"
...
assert config.protocol == io.FRESH_PROTOCOL
...
exec "$AF_PYTHON" scripts/submit_fresh_shared.py --root "$AF_OUTPUT_ROOT" --round "$round"
