...
export AF_SOURCE_ROOT="${AF_SOURCE_ROOT:-$group/detention-process-pilot-v7}"
export AF_AUDIT_ROOT="${AF_AUDIT_ROOT:-$group/basin-equation-audit-v1}"
export AF_OUTPUT_ROOT="${AF_OUTPUT_ROOT:-$group/basin-equation-repair-v1}"
...
cd "$AF_REPO_ROOT"
[[ -x "$AF_PYTHON" && -f "$AF_VLLM_IMAGE" ]]
"$AF_PYTHON" scripts/basin_repair_pilot.py prepare \
  --source "$AF_SOURCE_ROOT" --audit "$AF_AUDIT_ROOT" --root "$AF_OUTPUT_ROOT"
exec "$AF_PYTHON" scripts/submit_basin_repair_pilot.py --root "$AF_OUTPUT_ROOT"
