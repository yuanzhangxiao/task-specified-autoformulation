...
case "${protocol}" in
  detention-process-pilot-1) worker_script=detention_process_pilot.py ;;
...
if [[ ( "$protocol" == shared-process-pilot-1 || "$protocol" == detention-process-pilot-1 ) && -f "$AF_REPO_ROOT/SOURCE_COMMIT" ]]; then
