...
case "${protocol}" in
  ...
  review-deadline-1|review-deadline-2|review-deadline-3|review-deadline-4|review-deadline-5|review-deadline-6|review-deadline-7|review-deadline-8|shared-process-pilot-1|shared-process-integration-1) worker_script=review_deadline.py ;;
...
if [[ "${worker_script}" == staged_prefunction_campaign.py ]]; then
  "${AF_PYTHON}" - "${plan}" <<'PY'
import json
import sys
...
if [[ "${worker_script}" == staged_function_hybrid_campaign.py ]]; then
  "${AF_PYTHON}" - "${plan}" <<'PY'
import json
import sys
from autoformalism.rebuttal.staged_function_hybrid_campaign import hybrid_launcher_hash
with open(sys.argv[1], encoding="utf-8") as stream:
    frozen = json.load(stream)
...
if [[ "${worker_script}" == staged_function_prefit_campaign.py ]]; then
  "${AF_PYTHON}" - "${plan}" <<'PY'
import json
import sys
from autoformalism.rebuttal.staged_function_prefit_campaign import function_prefit_launcher_hash
with open(sys.argv[1], encoding="utf-8") as stream:
    frozen = json.load(stream)
...
import json
import sys
from autoformalism.rebuttal.staged_sign_contract_campaign import sign_contract_launcher_hash
with open(sys.argv[1], encoding="utf-8") as stream:
    frozen = json.load(stream)
if sign_contract_launcher_hash() != frozen["launcher_sha256"]:
    raise SystemExit("launcher differs from frozen sign-contract campaign")
PY
fi
if [[ "${worker_script}" == staged_polarity_calibration.py ]]; then
  "${AF_PYTHON}" - "${plan}" <<'PY'
import json
import sys
from autoformalism.rebuttal.staged_polarity_calibration import polarity_calibration_launcher_hash
with open(sys.argv[1], encoding="utf-8") as stream:
    frozen = json.load(stream)
if polarity_calibration_launcher_hash() != frozen["launcher_sha256"]:
    raise SystemExit("launcher differs from frozen polarity calibration")
PY
fi
...
if [[ ( "$protocol" == basin-equation-repair-1 || "$protocol" == shared-process-pilot-1 || "$protocol" == shared-process-integration-1 || "$protocol" == detention-process-pilot-1 || "$protocol" == detention-process-pilot-2 || "$protocol" == detention-process-pilot-3 ) && -f "$AF_REPO_ROOT/SOURCE_COMMIT" ]]; then
...
if [[ "$protocol" == review-deadline-7 || "$protocol" == review-deadline-8 ]]; then
