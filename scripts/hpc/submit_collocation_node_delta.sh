#!/bin/bash
# Import the portable public-only bundle and submit two policies for every case.
set -euo pipefail
...
export AF_OUTPUT_ROOT="${AF_OUTPUT_ROOT:-/work/hdd/bibo/yxiao2/phase_b/collocation-node-v1}"
...
"${AF_PYTHON}" scripts/run_collocation_node_campaign.py prepare --source "${AF_SOURCE_ROOT}" --config "${AF_CONFIG}" --output "${AF_OUTPUT_ROOT}"
...
write_json(path, {**previous, sys.argv[2]:sys.argv[3], "commit":sys.argv[4],
    "fit_tasks":len(frozen["tasks"]), "cases":len(frozen["cases"]),
...
fit="$(sbatch --parsable --array="0-${last}%${concurrency}" --output="${AF_OUTPUT_ROOT}/logs/fit-%A_%a.out" --error="${AF_OUTPUT_ROOT}/logs/fit-%A_%a.err" --export=ALL scripts/hpc/collocation_node_delta.slurm run)"
fit="${fit%%;*}"
record_job fit_job_id "${fit}"
summary="$(sbatch --parsable --dependency="afterany:${fit}" --time=00:30:00 --output="${AF_OUTPUT_ROOT}/logs/summary-%j.out" --error="${AF_OUTPUT_ROOT}/logs/summary-%j.err" --export=ALL scripts/hpc/collocation_node_delta.slurm summarize)"
summary="${summary%%;*}"
