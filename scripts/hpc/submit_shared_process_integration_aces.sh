...
set -euo pipefail
round="${1:-0}"
[[ "$#" -le 1 && "$round" =~ ^[01]$ ]] || { echo 'Usage: submit_shared_process_integration_aces.sh [0|1]' >&2; exit 2; }
...
export AF_OUTPUT_ROOT="${AF_OUTPUT_ROOT:-$group/shared-process-integration-v1}"
export AF_PUBLIC_ROOT="${AF_PUBLIC_ROOT:-/scratch/user/u.yx126462/phase_b/review-deadline-inputs-v1}"
export AF_CONFIG="$AF_REPO_ROOT/configs/shared_process_integration_v1.json"
...
assert config.protocol == io.INTEGRATION_PROTOCOL
