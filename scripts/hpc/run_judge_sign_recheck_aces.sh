#!/bin/bash
# Reuse the calibrated judge serving configuration; never start a proposer/fitter.
set -euo pipefail
...
stage="${1:?stage}"
cli=("$AF_PYTHON" "$AF_REPO_ROOT/scripts/judge_sign_recheck.py")
case "$stage" in
...
if [[ "$stage" == prepare ]]; then
  "$AF_PYTHON" -m pytest -q -p no:cacheprovider tests/test_judge_sign_correction.py tests/test_judge_sign_recheck.py tests/test_search_hybrid_pair.py
  "$AF_PYTHON" scripts/smoke_judge_sign_recheck.py
  "$AF_PYTHON" - <<'PY'
import os
from pathlib import Path
from autoformalism.rebuttal import judge_sign_recheck as campaign
assert campaign.content_hash(campaign.snapshot(Path(os.environ['AF_SOURCE_ROOT']))) == os.environ['AF_SIGN_SNAPSHOT_SHA256'], 'source receipts changed after submission'
PY
  "${cli[@]}" freeze --source "${AF_SOURCE_ROOT:?}" --root "$AF_OUTPUT_ROOT"
fi
...
if [[ "$stage" == prepare ]]; then
  ...
  exec "$runtime" exec --bind "$AF_HF_HOME:$AF_HF_HOME" --env "HF_HOME=$AF_HF_HOME,PYTHONDONTWRITEBYTECODE=1" \
    "$AF_VLLM_IMAGE" "$container_python" -c 'from huggingface_hub import snapshot_download; import sys; snapshot_download("openai/gpt-oss-120b", revision=sys.argv[1])' "$revision"
fi
cache="$(mktemp -d /tmp/af-sign-recheck-XXXXXX)"
server_pid=''
cleanup() {
  status=$?
  trap - EXIT
...
"$runtime" exec --nv --bind "$AF_HF_HOME:$AF_HF_HOME,$cache:$cache" \
  ...
  --tensor-parallel-size 2 --max-model-len 32768 --max-num-seqs 1 --gpu-memory-utilization 0.90 > "$log" 2>&1 &
server_pid=$!
ready=false
