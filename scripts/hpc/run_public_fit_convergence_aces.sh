#!/bin/bash
# One CPU, a bounded convergence diagnostic, with both historical roots read-only.
set -euo pipefail
: "${AF_REPO_ROOT:?required}" "${AF_OUTPUT_ROOT:?required}" "${AF_PYTHON:?required}"
: "${AF_PARENT_FIT:?required}" "${AF_CONTINUATION_FIT:?required}" "${AF_SELECTION:?required}"
module load GCCcore/13.2.0 Python/3.11.5
file_sha() { sha256sum "$1" | cut -d' ' -f1; }
verify_submission() {
  local manifest="$AF_OUTPUT_ROOT/submission-intent/identity.json" artifact field source source_path
  ...
  [[ "$AF_CONTINUATION_FIT" == "$(jq -er '.continuation_fit' "$manifest")" ]] || { echo 'Continuation path changed after submission' >&2; return 2; }
  # Recheck the whole experiment boundary even for a manually supplied old intent.
  "$AF_PYTHON" - "$AF_REPO_ROOT" "$AF_OUTPUT_ROOT" "$AF_PARENT_FIT" "$AF_CONTINUATION_FIT" "$AF_SELECTION" <<'PY' || return 2
import sys
...
PY
  ...
  for source in parent continuation; do
    if [[ "$source" == parent ]]; then source_path="$AF_PARENT_FIT"; else source_path="$AF_CONTINUATION_FIT"; fi
    for artifact in freeze result backend_result; do
      case "$artifact" in
        freeze) field="${source}_freeze_sha256" ;;
        result) field="${source}_result_sha256" ;;
        backend_result) field="${source}_backend_sha256" ;;
      esac
      [[ -f "$source_path/$artifact.json" && "$(file_sha "$source_path/$artifact.json")" == "$(jq -er ".$field" "$manifest")" ]] || { echo "$source $artifact changed after submission" >&2; return 2; }
    done
  done
...
if "$AF_PYTHON" -m pytest -q -p no:cacheprovider tests/test_public_fit_convergence.py tests/test_public_fit_convergence_submission.py > "$log" 2>&1; then
  tail -n 2 "$log"
else
  status=$?
  tail -n 80 "$log" >&2
...
"$AF_PYTHON" scripts/smoke_public_fit_convergence.py > "$AF_OUTPUT_ROOT/runtime/smoke.json"
verify_submission
directory="$AF_OUTPUT_ROOT/diagnostic"
"$AF_PYTHON" scripts/run_public_fit_convergence.py prepare --parent "$AF_PARENT_FIT" --continuation "$AF_CONTINUATION_FIT" --selection "$AF_SELECTION" --output "$directory" > "$AF_OUTPUT_ROOT/runtime/prepared.json"
"$AF_PYTHON" scripts/run_public_fit_convergence.py inspect --output "$directory" > "$AF_OUTPUT_ROOT/runtime/inspection.json"
"$AF_PYTHON" scripts/run_public_fit_convergence.py run --output "$directory" > "$AF_OUTPUT_ROOT/runtime/run.json"
"$AF_PYTHON" scripts/run_public_fit_convergence.py report --output "$directory" > "$AF_OUTPUT_ROOT/summary.json.tmp"
