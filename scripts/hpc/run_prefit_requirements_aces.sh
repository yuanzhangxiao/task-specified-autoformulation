#!/bin/bash
# CPU reconstruction/freeze and preparation; GPU local repair. There is no fitting stage.
set -euo pipefail
...
if [[ "$mode" == prepare ]]; then
  ...
  "$AF_PYTHON" scripts/prefit_requirement_campaign.py verify --root "$AF_OUTPUT_ROOT"
  module load WebProxy
