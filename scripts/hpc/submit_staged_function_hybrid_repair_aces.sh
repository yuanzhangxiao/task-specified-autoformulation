#!/bin/bash
# Freeze and submit the opaque-only batched-function/atomic-repair experiment.
set -euo pipefail
...
"${python}" "${repository}/scripts/staged_function_hybrid_campaign.py" freeze --config "${config}" --source-function-plan "${AF_SOURCE_FUNCTION_PLAN}" --output "${plan}"
if [[ -f "${manifest}" ]]; then
  cat "${manifest}"
  exit 0
fi
...
job_id="$(
  sbatch --parsable --account="${account}" --open-mode=append \
    --output="${output_root}/logs/staged-function-hybrid-%j.out" \
    --error="${output_root}/logs/staged-function-hybrid-%j.err" \
    ...
    "${repository}/scripts/hpc/staged_function_hybrid_repair_aces.slurm"
)"
job_id="${job_id%%;*}"
...
PY
printf 'ACES_FUNCTION_HYBRID_JOB=%s\nACES_FUNCTION_HYBRID_ROOT=%s\n' "${job_id}" "${output_root}"
