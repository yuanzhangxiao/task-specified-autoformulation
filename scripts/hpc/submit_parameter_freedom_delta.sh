#!/bin/bash
# New comparisons only; never reset an interrupted native solver's budget.
set -euo pipefail
...
export AF_SOURCE_ROOT="${AF_SOURCE_ROOT:-/work/hdd/bibo/yxiao2/phase_b/fitter-resolution-v3}"
export AF_OUTPUT_ROOT="${AF_OUTPUT_ROOT:-/work/hdd/bibo/yxiao2/phase_b/fitter-parameter-freedom-v1}"
...
"${AF_PYTHON}" scripts/run_parameter_freedom.py prepare --source "${AF_SOURCE_ROOT}" --output "${AF_OUTPUT_ROOT}" --config "${AF_CONFIG:-configs/fitter_parameter_freedom_v1.json}"
if [[ -f "${AF_OUTPUT_ROOT}/submission.json" ]]; then
  cat "${AF_OUTPUT_ROOT}/submission.json"
  echo 'Existing submission retained; reconcile any incomplete submission with the queue.'
  exit 0
fi
...
smoke="$(submit smoke --time=00:10:00 scripts/hpc/parameter_freedom_delta.slurm smoke)"
gate="$(submit gate --time=00:10:00 --dependency="afterok:${smoke}" --kill-on-invalid-dep=yes scripts/hpc/parameter_freedom_delta.slurm gate)"
fit="$(submit fit --dependency="afterok:${gate}" --kill-on-invalid-dep=yes --array="0-5%${concurrency}" scripts/hpc/parameter_freedom_delta.slurm run)"
submit summary --time=00:10:00 --dependency="afterany:${fit}:${gate}" --kill-on-invalid-dep=yes scripts/hpc/parameter_freedom_delta.slurm summarize >/dev/null
cat "${AF_OUTPUT_ROOT}/submission.json"
