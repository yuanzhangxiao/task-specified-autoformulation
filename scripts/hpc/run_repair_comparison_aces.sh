...
if [[ "${1:-}" == prepare ]]; then
  ...
  cd "$AF_REPO_ROOT"
  "$AF_PYTHON" -m pytest -q tests/test_repair_comparison.py tests/test_repair_drafts.py tests/test_repair_action_replay.py > "$AF_OUTPUT_ROOT/runtime/preflight-tests-${SLURM_JOB_ID}.log" 2>&1
