#!/bin/bash
# Idempotent two-job ACES pilot: CPU reconstruction/preflight, then one 20B H100 server.
set -euo pipefail
...
worker="$repo/scripts/hpc/run_prefit_requirements_aces.sh"
if [[ "$resume" == 1 ]]; then
  [[ -f "$manifest" && -f "$root/plan.json" ]] || { echo 'Resume requires a submitted, frozen experiment' >&2; exit 2; }
  "$python" "$repo/scripts/prefit_requirement_campaign.py" verify --root "$root"
  ...
  "$python" "$repo/scripts/prefit_requirement_campaign.py" summary --root "$root" > "$root/submissions/resume-summary.json"
