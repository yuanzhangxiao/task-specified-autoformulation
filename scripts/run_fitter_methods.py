#!/usr/bin/env python3
"""Freeze, run and summarize the user-submitted fitting-method comparison."""
...
from autoformalism.rebuttal.fitter_methods import (  # noqa: E402
    MethodsPlan,
    execute_methods,
    prepare_methods,
    summarize_methods,
...
def main() -> None:
    ...
    if args.stage == "prepare":
        ...
        with (output / "prepare.lock").open("w") as lock:
            ...
            frozen = prepare_methods(
                MethodsPlan.model_validate(read_json(args.config)), output
            )
        print(frozen["freeze_sha256"], "tasks", len(frozen["tasks"]))
    elif args.stage == "summarize":
        summarize_methods(output)
        ...
        with (root / lock_name).open("w") as lock:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
            result = (run_supervised if args.stage == "run" else execute_methods)(
