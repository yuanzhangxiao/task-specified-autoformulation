#!/usr/bin/env python3
"""CPU submission with durable per-job receipts and verified recovery."""
...
from autoformalism.rebuttal import dalla_rescue as io
...
def submit(source: Path, root: Path, adopt: dict | None = None) -> dict:
    ...
    plan = io.freeze(source, root)
    identity = {"commit": commit, "plan_sha256": plan["artifact_sha256"]}
    adopt = adopt or {}
    if set(adopt) - {"prepare", "fit", "report"}:
    ...
    with public._lock(root / "scheduler"):
        ...
        worker = io.REPO / "scripts/hpc/run_dalla_rescue_aces.sh"
        ...
        result = {
            ...
            "identity": identity,
            "jobs": {"prepare": prepare, "fit": fit, "report": report},
...
if __name__ == "__main__":
    ...
    print(
        json.dumps(
            submit(args.inputs, args.root, dict(x.split("=", 1) for x in args.adopt)),
            indent=2,
        )
    )
