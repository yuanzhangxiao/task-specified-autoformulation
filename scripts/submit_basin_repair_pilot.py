#!/usr/bin/env python3
"""Submit one saved-basin repair episode with immutable scheduler receipts."""
...
from autoformalism.rebuttal import basin_repair_pilot as io
...
def submit(root: Path, index: int = 0) -> dict:
    ...
    root = root.resolve()
    plan = io.verify(root)
    if plan["protocol"] != io.PROTOCOL or index != 0:
    ...
    with public._lock(root / "scheduler"):
        ...
        worker = repo / "scripts/hpc/run_basin_repair_pilot_aces.sh"
        jobs = {}
        ...
        value["maximum_repair_calls"] = len(plan["tasks"]) * plan["max_repair_calls"]
        value["maximum_child_fits"] = len(plan["tasks"])
