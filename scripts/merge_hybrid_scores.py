...
def main() -> None:
    ...
    for input_index, path in enumerate(args.inputs):
        ...
        frames.append(frame)
    if not frames:
        raise SystemExit("no hybrid score shards were found")
    ...
    if len(merged) != args.expected:
        raise SystemExit(f"incomplete hybrid scores: {len(merged)}/{args.expected}")
    args.output.parent.mkdir(parents=True, exist_ok=True)
    merged.drop(columns=["_input_index", "_source_row"]).to_csv(
        args.output, index=False
    )
    print(
        f"merged {len(merged)} unique hybrid comparisons into {args.output}; "
