#!/usr/bin/env python3
"""Freeze, run and summarize the user-submitted active-dynamics recovery test."""
...
from autoformalism.rebuttal.fitter_recovery import (  # noqa: E402
    RecoveryPlan,
    execute_recovery,
    prepare_recovery,
    summarize_recovery,
...
def main() -> None:
    ...
    if args.stage == "prepare":
        ...
        with (output / "prepare.lock").open("w") as lock:
            ...
            frozen = prepare_recovery(
                RecoveryPlan.model_validate(read_json(args.config)), output
            )
        print(frozen["freeze_sha256"], "tasks", len(frozen["tasks"]))
    elif args.stage == "summarize":
        summarize_recovery(output)
        ...
        with (root / lock_name).open("w") as lock:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
            result = (run_supervised if args.stage == "run" else execute_recovery)(
